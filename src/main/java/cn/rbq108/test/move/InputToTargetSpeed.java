...
public class InputToTargetSpeed {
    ...
    public static Vec3 calculateVelocity(
        ...
        // 1. 获取输入向量 (int 会自动隐式转换为 double)
        Vec3 rawInput = new Vec3(B_INx, B_INy, B_INz);

        // 2. 计算输入强度，并限制最大为 1.0
        // (防止同时按 W 和 D 时模长变成 1.414 导致斜跑过快)
